...
final class Cookie {
    ...
    public function __construct($name, $domain=null) {
        ...
        $this->expiryTime = 0;
        $this->path = '/';
        $this->domain = self::normalizeDomain($domain?$domain:$_SERVER['HTTP_HOST']);
        $this->httpOnly = true;
     ...
     *
     * @param null $name
     * @return bool TRUE if cookie exists; FALSE if cookie doesn't exist
     */
    ...
    public function getValue($name=null) {
        if (!$this->value) {
            $value = $_COOKIE[$name?$name:$this->name];
            if (is_array($value)) {
    ...
    public function save() {
        self::_setcookie($this->name, $this->value, $this->expiryTime, $this->path, $this->domain, $this->secureOnly, $this->httpOnly);
        $passed = self::addHttpHeader((string) $this);
        return $passed;
    }
    ...
    public function delete($name=null) {
        if ($name) {
            $clonedCookie = clone $this;
            $clonedCookie->name = $name;
            return $clonedCookie->setValue('')->setMaxAge(0)->save();
        }
        return $this->setValue(null)->setMaxAge(0)->save();
    }
     ...
     * @return \Ezpz\Lib\Cookie|null the cookie instance or `null`
     */
    ...
    public static function get($name) {
        return self::has($name) ? filter_input(INPUT_COOKIE, $name) : null;
    }
    ...
    public static function set($name, $value = null, $expiryTime = 0, $path = null, $domain = null, $secureOnly = false, $httpOnly = false, $sameSiteRestriction = null) {
        self::setcookie($name, $value, $expiryTime, $path, $domain, $secureOnly, $httpOnly, $sameSiteRestriction);
    }

    private static function _setCookie($name, $value = null, $expiryTime = 0, $path = null, $domain = null, $secureOnly = false, $httpOnly = false) {
        if (!$value) {
            $currentValue = self::get($name);
            if ($currentValue && is_array($currentValue)) {
                foreach ($currentValue as $key=>$val) {
                    setcookie($name.'['.$key.']', $value, $expiryTime, $path, $domain, $secureOnly, $httpOnly);
                }
            }
            else {
                setcookie($name, $value, $expiryTime, $path, $domain, $secureOnly, $httpOnly);
            }
        }
        else if (is_object($value) || is_array($value)) {
            foreach ($value as $key=>$val) {
                $val = is_object($val)||is_array($val)?json_encode($val):$val;
                setcookie($name.'['.$key.']', $val, $expiryTime, $path, $domain, $secureOnly, $httpOnly);
            }
        }
        else {
            setcookie($name, $value, $expiryTime, $path, $domain, $secureOnly, $httpOnly);
